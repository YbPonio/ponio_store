export const ICONS = {
  pos: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="16" height="20" x="4" y="2" rx="2"/><line x1="8" x2="16" y1="6" y2="6"/><line x1="16" x2="16" y1="14" y2="18"/><path d="M16 10h.01"/><path d="M12 10h.01"/><path d="M8 10h.01"/><path d="M12 14h.01"/><path d="M8 14h.01"/><path d="M12 18h.01"/><path d="M8 18h.01"/></svg>`,
  inventory: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/></svg>`,
  orders: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1Z"/><path d="M14 8H8"/><path d="M16 12H8"/><path d="M13 16H8"/></svg>`,
  ...
  adjust: `<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 16 4 4 4-4"/><path d="M7 20V4"/><path d="m21 8-4-4-4 4"/><path d="M17 4v16"/></svg>`,
  ...
  logOut: `<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg>`,
...
export function emitEvent(target, eventName, detail = null, bubbles = true) {
...
export function showToast(message, type = 'info', duration = 3000) {
  ...
  if (!container) {
    ...
    container.className = 'fixed bottom-16 md:bottom-5 left-3 md:left-5 z-50 flex flex-col items-start gap-2 pointer-events-none max-w-[calc(100vw-1.5rem)] sm:max-w-sm';
  ...
  const icons = {
    ...
    error: ICONS.x,
    warning: ICONS.alert,
    info: `<span class="w-1.5 h-1.5 rounded-full bg-slate-600"></span>`,
  };

  toast.className = `pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-xl bg-white border border-slate-200 text-slate-800 shadow-lg shadow-slate-200/60 transition-all duration-200 transform translate-y-2 opacity-0 text-xs font-semibold select-none`;
  toast.innerHTML = `
    <span class="w-5 h-5 flex items-center justify-center rounded-lg text-xs font-bold shrink-0 ${typeBadge[type] || typeBadge.info}">
      ${icons[type] || ''}
    </span>
...
export function confirmDialog({
  ...
  return new Promise((resolve) => {
    ...
    dialog.innerHTML = `
      <div class="flex flex-col p-4 sm:p-5 bg-white">
        <div class="flex items-start gap-3 mb-3">
          <div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${isDestructive ? 'bg-rose-50 text-rose-600 border border-rose-200' : 'bg-slate-100 text-slate-700 border border-slate-200'}">
            ${isDestructive ? ICONS.alert : ICONS.shield}
          </div>
          <div class="flex-1 min-w-0">
            <h3 class="text-xs sm:text-sm font-bold text-slate-900 leading-snug">${escapeHtml(title)}</h3>
            <p class="text-[11px] sm:text-xs text-slate-500 mt-1 leading-relaxed">${escapeHtml(message)}</p>
          </div>
        </div>

        <div class="flex items-center justify-end gap-2 mt-2 pt-2 border-t border-slate-100">
          <button id="modal-confirm-cancel" type="button" class="px-3.5 py-1.5 rounded-xl border border-slate-200 hover:bg-slate-100 active:bg-slate-200 text-slate-700 font-semibold text-xs transition-all cursor-pointer">
            ${escapeHtml(cancelText)}
          </button>
          <button id="modal-confirm-proceed" type="button" class="px-3.5 py-1.5 rounded-xl font-bold text-xs shadow-xs transition-all cursor-pointer ${isDestructive ? 'bg-rose-600 hover:bg-rose-700 active:bg-rose-800 text-white' : 'bg-slate-900 hover:bg-slate-800 active:bg-slate-950 text-white'}">
    ...
    const cancelBtn = dialog.querySelector('#modal-confirm-cancel');
    const proceedBtn = dialog.querySelector('#modal-confirm-proceed');
